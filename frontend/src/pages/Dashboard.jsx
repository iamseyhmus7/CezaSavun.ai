...
import { generatePetition, apiFetch } from '../utils/api';
...
export default function Dashboard() {
  const { currentUser, petitions, filteredPetitions, loadData, loadingList } = useOutletContext();
  ...
  const handleFile = async (file) => {
    ...
    setAnalysisPhase(0);
    addLog(`${file.name} belgesi yüklendi, OCR başlatılıyor...`, 'info');

    const interval = setInterval(() => {
      setAnalysisPhase(prev => (prev >= 3 ? 3 : prev + 1));
    }, 2500);

    try {
      addLog('Gemini Vision ile OCR analizi yapılıyor...', 'info');
      const data = await generatePetition(file, clientName);
      setLastPetitionId(data.petition_id);
      setLastPetitionData(data);
      addLog('AI dilekçe yazımı tamamlandı, DB\'ye kaydedildi.', 'success');
      loadData(); // Yenile
    } catch (e) {
  ...
  const handleDownloadRaw = () => {
    if (!lastPetitionData?.draft_petition) return;
    const el   = document.createElement('a');
    const blob = new Blob([lastPetitionData.draft_petition], { type: 'text/plain;charset=utf-8' });
    el.href    = URL.createObjectURL(blob);
    el.download = `itiraz_dilekcesi_${uploadedFileName.split('.')[0]}.txt`;
    el.click();
  };
    ...
    <div className="space-y-8 flex flex-col h-full">
       ...
       >
         ...
         <div className="flex flex-col items-center">
            ...
            <p className="text-[10px] text-slate-400 font-black tracking-widest uppercase">{isProcessing ? `AŞAMA ${analysisPhase + 1}/4` : 'PDF VEYA GÖRSEL'}</p>
         </div>
       </div>
